...
class TurtleBot:
    ...
    def __init__(self):
        ...
        rospy.init_node('spain_controller', anonymous=False)

        self.comm_vel_subscriber = rospy.Subscriber('cmd_vel_mux/input/navi',
                                                    Twist, self.wheel_calc)

        self.kinematic_publisher = rospy.Publisher(
            'DIFFERENTIAL_INVERSE_KINEMATICS_OUTPUT', Vector3, queue_size=10)
    ...
    def wheel_calc(self, data):
        ...
        d_phi = Vector3()
        d_phi.x = self.twist.linear.x * \
            (1 / r) + self.twist.angular.z * (b / r)
        d_phi.y = self.twist.linear.x * \
            (1 / r) + self.twist.angular.z * (-b / r)
        d_phi.z = 0
